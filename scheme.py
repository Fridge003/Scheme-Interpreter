...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # turning scheme_eval into a single-input function
        def map_eval(expr):
            return scheme_eval(expr, env, _=None)
        # evaluating a call-expression, proc: the operator, args: the operands
        # BEGIN PROBLEM 4
        proc = scheme_eval(first, env)
        validate_procedure(proc)
        args = rest.map(map_eval)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        else: # correct number of values
            child_frame = Frame(self)
            formal, val = formals, vals # bind formal-value pairs in the new env
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("TypeError when calling builtin function: ", str(self.fn))
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        try:
            ...
            raise SchemeError('The number of operands must be 2')
        # BEGIN PROBLEM 5
        # call the define method of class Frame to add bindings
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        if is_false_primitive(scheme_eval(expressions.first, env)):
            return False
        expressions = expressions.rest
    final_element = scheme_eval(expressions.first, env)
    if is_false_primitive(final_element):
        return False
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    while expressions.rest is not nil:
        cur_element = scheme_eval(expressions.first, env)
        if is_true_primitive(cur_element):
            return cur_element
        expressions = expressions.rest
    final_element = scheme_eval(expressions.first, env)
    if is_true_primitive(final_element):
        return final_element
    else:
        return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    validate_formals(names) # check whether names are distinct
    # END PROBLEM 14
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        of values, for a dynamically-scoped call evaluated in parent environment."""
        # BEGIN PROBLEM 11
